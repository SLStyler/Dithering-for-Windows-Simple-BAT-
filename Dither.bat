...
([System.Management.Automation.PsEngineEvent]::Exiting) -Action { clear;Write-Host "$([char]27)[1;34mMade by $([char]27)[1;36mSL$([char]27)[0m" }
$addcounter = 0
...
$GetFolder = Get-ChildItem -Path $Path  -exclude C*,A* |Select-Object Name | Split-Path -Leaf
   ...
   if (($regcheck -eq $true) -and $switcher -eq $true) {
    ...
    elseif(($regcheck -eq $false) -or ($regcheck -eq $true -and $switcher -eq $false)){
        ...
        switch($entry,$NoneValue) {
            ...
            T6{Remove-ItemProperty  -Path Registry::HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Services\nvlddmkm\State\DisplayDatabase\$GetFolder -Name $valuename;New-ItemProperty -Path $result -Name $valuename -PropertyType Binary -Value $TempValue6bit;Write-Host "`n`n$esc[1;36m$Temporal6$esc[0m$esc[1;21m"was successfully created"$esc[0m";CloseAppOrReg;exit           }
            T8{Remove-ItemProperty  -Path Registry::HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Services\nvlddmkm\State\DisplayDatabase\$GetFolder -Name $valuename;New-ItemProperty -Path $result -Name $valuename -PropertyType Binary -Value $TempValue8bit ;Write-Host "`n`n$esc[1;36m$Temporal8$esc[0m$esc[1;21m"was successfully created"$esc[0m";CloseAppOrReg;exit           }
            T10{Remove-ItemProperty  -Path Registry::HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Services\nvlddmkm\State\DisplayDatabase\$GetFolder -Name $valuename;New-ItemProperty -Path $result -Name $valuename -PropertyType Binary -Value $TempValue10bit;Write-Host "`n`n$esc[1;36m$Temporal10$esc[0m$esc[1;21m"was successfully created"$esc[0m";CloseAppOrReg;exit           }
            S6{Remove-ItemProperty  -Path Registry::HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Services\nvlddmkm\State\DisplayDatabase\$GetFolder -Name $valuename;New-ItemProperty -Path $result -Name $valuename -PropertyType Binary -Value $StaticValue6bit ;Write-Host "`n`n$esc[1;36m$Static6$esc[0m$esc[1;21m"was successfully created"$esc[0m";CloseAppOrReg;exit             }
            S8{Remove-ItemProperty  -Path Registry::HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Services\nvlddmkm\State\DisplayDatabase\$GetFolder -Name $valuename;New-ItemProperty -Path $result -Name $valuename -PropertyType Binary -Value $StaticValue8bit ;Write-Host "`n`n$esc[1;36m$Static8$esc[0m$esc[1;21m"was successfully created"$esc[0m";CloseAppOrReg;exit             }
            S10{Remove-ItemProperty  -Path Registry::HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Services\nvlddmkm\State\DisplayDatabase\$GetFolder -Name$valuename;New-ItemProperty -Path $result -Name $valuename -PropertyType Binary -Value $StaticValue10bit ;Write-Host "`n`n$esc[1;36m$Static10$esc[0m$esc[1;21m"was successfully created"$esc[0m";CloseAppOrReg;exit             }
            D6{Remove-ItemProperty  -Path Registry::HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Services\nvlddmkm\State\DisplayDatabase\$GetFolder -Name $valuename;New-ItemProperty -Path $result -Name $valuename -PropertyType Binary -Value $DynamicValue6bit ;Write-Host "`n`n$esc[1;36m$Dynamic6$esc[0m$esc[1;21m"was successfully created"$esc[0m";CloseAppOrReg;exit            }
            D8{Remove-ItemProperty  -Path Registry::HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Services\nvlddmkm\State\DisplayDatabase\$GetFolder -Name $valuename;New-ItemProperty -Path $result -Name $valuename -PropertyType Binary -Value $DynamicValue8bit ;Write-Host "`n`n$esc[1;36m$Dynamic8$esc[0m$esc[1;21m"was successfully created"$esc[0m";CloseAppOrReg;exit            }
            D10{Remove-ItemProperty  -Path Registry::HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Services\nvlddmkm\State\DisplayDatabase\$GetFolder -Name $valuename;New-ItemProperty -Path $result -Name $valuename -PropertyType Binary -Value $DynamicValue10bit ;Write-Host "`n`n$esc[1;36m$Dynamic10$esc[0m$esc[1;21m"was successfully created"$esc[0m";CloseAppOrReg;exit            }
