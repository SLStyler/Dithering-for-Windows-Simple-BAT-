...
Get-Process -name cmd.exe| Where-Object -FilterScript {$_.Responding -eq $false} | Stop-Process
Set-psreadlinekeyhandler -key escape -function viexit
...
$GetFolder = Get-ChildItem -Recurse -Path $Path -Include $GetDisplay.ForEach({ $_ + '*'}) |Select-Object Name |Split-Path -Leaf
...
$subkey = [Microsoft.Win32.Registry]::LocalMachine.OpenSubKey("SYSTEM\CurrentControlSet\Services\nvlddmkm\State\DisplayDatabase",[Microsoft.Win32.RegistryKeyPermissionCheck]::ReadWriteSubTree,[System.Security.AccessControl.RegistryRights]::ChangePermissions)
...
$result = "Registry::HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Services\nvlddmkm\State\DisplayDatabase\$GetFolder"
   ...
   if (($regcheck -eq $true) -and $switcher -eq $true) {
    ...
    elseif(($regcheck -eq $false) -or ($regcheck -eq $true -and $switcher -eq $false)){
        ...
        switch($entry,$NoneValue) {
            ...
            T6{$GetFolder.ForEach({Remove-ItemProperty  -Path Registry::HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Services\nvlddmkm\State\DisplayDatabase\$_ -Name $valuename});$GetFolder.ForEach({New-ItemProperty -Path $result -Name $valuename -PropertyType Binary -Value $TempValue6bit;Write-Host "`n`n$esc[1;36m$Temporal6$esc[0m$esc[1;21m"was successfully created"$esc[0m"});CloseAppOrReg;exit           }
            T8{$GetFolder.ForEach({Remove-ItemProperty  -Path Registry::HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Services\nvlddmkm\State\DisplayDatabase\$_ -Name $valuename});$GetFolder.ForEach({New-ItemProperty -Path $result -Name $valuename -PropertyType Binary -Value $TempValue8bit ;Write-Host "`n`n$esc[1;36m$Temporal8$esc[0m$esc[1;21m"was successfully created"$esc[0m"});CloseAppOrReg;exit           }
            T10{$GetFolder.ForEach({Remove-ItemProperty  -Path Registry::HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Services\nvlddmkm\State\DisplayDatabase\$_ -Name $valuename});$GetFolder.ForEach({New-ItemProperty -Path $result -Name $valuename -PropertyType Binary -Value $TempValue10bit;Write-Host "`n`n$esc[1;36m$Temporal10$esc[0m$esc[1;21m"was successfully created"$esc[0m"});CloseAppOrReg;exit           }
            S6{$GetFolder.ForEach({Remove-ItemProperty  -Path Registry::HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Services\nvlddmkm\State\DisplayDatabase\$_ -Name $valuename});$GetFolder.ForEach({New-ItemProperty -Path $result -Name $valuename -PropertyType Binary -Value $StaticValue6bit ;Write-Host "`n`n$esc[1;36m$Static6$esc[0m$esc[1;21m"was successfully created"$esc[0m"});CloseAppOrReg;exit             }
            S8{$GetFolder.ForEach({Remove-ItemProperty  -Path Registry::HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Services\nvlddmkm\State\DisplayDatabase\$_ -Name $valuename});$GetFolder.ForEach({New-ItemProperty -Path $result -Name $valuename -PropertyType Binary -Value $StaticValue8bit ;Write-Host "`n`n$esc[1;36m$Static8$esc[0m$esc[1;21m"was successfully created"$esc[0m"});CloseAppOrReg;exit             }
            S10{$GetFolder.ForEach({Remove-ItemProperty  -Path Registry::HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Services\nvlddmkm\State\DisplayDatabase\$_ -Name $valuename});$GetFolder.ForEach({New-ItemProperty -Path $result -Name $valuename -PropertyType Binary -Value $StaticValue10bit ;Write-Host "`n`n$esc[1;36m$Static10$esc[0m$esc[1;21m"was successfully created"$esc[0m"});CloseAppOrReg;exit             }
            D6{$GetFolder.ForEach({Remove-ItemProperty  -Path Registry::HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Services\nvlddmkm\State\DisplayDatabase\$_ -Name $valuename});$GetFolder.ForEach({New-ItemProperty -Path $result -Name $valuename -PropertyType Binary -Value $DynamicValue6bit ;Write-Host "`n`n$esc[1;36m$Dynamic6$esc[0m$esc[1;21m"was successfully created"$esc[0m"});CloseAppOrReg;exit            }
            D8{$GetFolder.ForEach({Remove-ItemProperty  -Path Registry::HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Services\nvlddmkm\State\DisplayDatabase\$_ -Name $valuename});$GetFolder.ForEach({New-ItemProperty -Path $result -Name $valuename -PropertyType Binary -Value $DynamicValue8bit ;Write-Host "`n`n$esc[1;36m$Dynamic8$esc[0m$esc[1;21m"was successfully created"$esc[0m"});CloseAppOrReg;exit            }
            D10{$GetFolder.ForEach({Remove-ItemProperty  -Path Registry::HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Services\nvlddmkm\State\DisplayDatabase\$_ -Name $valuename});$GetFolder.ForEach({New-ItemProperty -Path $result -Name $valuename -PropertyType Binary -Value $DynamicValue10bit ;Write-Host "`n`n$esc[1;36m$Dynamic10$esc[0m$esc[1;21m"was successfully created"$esc[0m"});CloseAppOrReg;exit            }
...
if ($counter -le 2){
  switch($entrychecker){    
     ...
     Delete{Write-Host "`n$esc[1;31mENTRY WAS DELETED`n $esc[0m";Remove-ItemProperty  -Path Registry::HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Services\nvlddmkm\State\DisplayDatabase\$GetFolder -Name DitherRegistryKey;CloseAppOrReg }
